  ...
  return <div className="site-shell">
    ...
    <main id="main">
      <section className="editorial-hero" aria-labelledby="hero-title">
        ...
        <div className="hero-copy"><p className="eyebrow">Keeanu Contreras · Fotografía & video</p><p className="hero-mark" aria-hidden="true">K</p><h1 id="hero-title">Fotografía<br/>que se siente.</h1><p>Recuerdos honestos, luz natural y una mirada cercana para contar lo que de verdad importa.</p><a className="arrow-link" href="#trabajo">Descubrir el trabajo</a></div>
      ...
      <section className="services"><header><p className="section-index">04 / Lo que hacemos</p><h2>Cada historia<br/>merece su lenguaje.</h2><p className="location-note">Desde Sonora hacia donde nos lleve la historia.</p></header><div className="service-list">{categories.map((category) => <div key={category.id} id={category.id}><span>{category.number}</span><h3>{category.title}</h3><p>{category.text}</p></div>)}</div></section>
      <section id="paquetes" className="packages"><header><p className="section-index">04 / Paquetes base</p><h2>El punto de partida<br/><em>para su historia.</em></h2><p>Elegimos la cobertura que mejor acompaña su día y ajustamos cada detalle a lo que quieren recordar.</p></header><div className="package-grid">{packages.map((item) => <article key={item.number}><span>{item.number}</span><h3>{item.title}</h3><p>{item.text}</p><a href="#contact">Solicitar información</a></article>)}</div><p className="package-note">También podemos sumar horas, un segundo fotógrafo, photobook o foto enmarcada según lo que necesiten.</p></section>
